      ...
      sets:from_list(
        lists:map(fun({attribute, _Line1, file, {File, _Line2}}) ->
                  ...
                  lists:filter(fun({attribute, _Line1, file, {_File, _Line2}}) ->
                               ...
                               Form)))),
  io:format("~p ~p ~n", [SrcFile, DepFile]),
...
compile_c_linux(File, debug) ->
  ...
  case is_c_file_changed(Target, DepFile) of
    true ->
      ...
      Cmd = io_lib:format("g++ -c \"~s\" -o \"~s\" ~s", [File, Target, Opts]),
      ok = exec(Cmd, true),
      true;
    false ->
      false
...
build_c_test_linux(TestSrc, Mode) ->
  ...
  case check_dependencies(Target, [KdbLib | Objects00]) of
    true ->
      ...
      Cmd = io_lib:format("g++ -static -static-libgcc -Xlinker -zmuldefs -o \"~s\" ~s ~s \"~s\" ~s", [Target, ModeOpts, Objects10, KdbLib, Libs]),
      ok = exec(Cmd, true);
    false ->
      ok
  end.
