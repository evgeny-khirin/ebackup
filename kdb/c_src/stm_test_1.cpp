...
/// Description : Unit test for STM. Test checks correct program behaivior
...
void start_devs() {
	...
	// start ld_bd device
	if (!ld_bd::is_formatted("log_bd_cache", "STM_TEST_1")) {
		ld_bd::format("log_bd_cache", "STM_TEST_1");
	}
	...
	uint64_t cp_log_size = g_log_capacity / 16 * g_log_block_size;
...
/*
	if (random_value_1(1000) == 1) {
		bd_crash * pcrash = (bd_crash *)whereis("log_bd_crash");
		pcrash->ops(true);
		pcrash = (bd_crash *)whereis("data_bd_crash");
		pcrash->ops(true);
		throw power_failure();
	}
*/
