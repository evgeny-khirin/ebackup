...
/// Description : Unit test for ld_pp. Test checks correct program behaivior
...
void start_devs() {
	...
	// start ld_bd device
	if (!ld_bd::is_formatted("bd_cache", "LD_PP_TEST")) {
		ld_bd::format("bd_cache", "LD_PP_TEST");
	}
...
void init_buffer(int32_t v, temp_buffer & buffer) {
	buffer.alloc(32 + random_value(32));
